...
const {Houses,Enquiries,Users} = require("../model/allSchemas");
...
allrouter.get("/searchresults/:county", async (request, response) => {
  const house = await Houses.find({county:request.params.county});
  try {
    response.send(house);
    console.log(house)
  } catch (error) {
...
// to get the house based on id of the house
